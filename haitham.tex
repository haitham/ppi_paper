...
is discarding available knowledge of the network topology and colors already
assigned to its nodes.

As guided by Gulsoy et al.\cite{gulsoy}, knowing the network topology can be
...
a solid lower bound on $P_s$. For a given node $v$ in a given path, all
$max\_k(v)$ nodes in either direction from $v$ are not allowed to have the same
color as $v$. We represent this rule as an unweighted constraint graph $W = (H,
L)$ where $H$ is the set of nodes and $L$ is the set of edges. $H$ contains a
...
\begin{figure}[h]
\centerline{\psfig{file=maxk.eps,width=15cm}}
